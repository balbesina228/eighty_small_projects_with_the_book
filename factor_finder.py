...
print('''
Factor Finder, by Al Sweigart. (made a little modification by myself)
...
while True:
    response = input('Enter a number or "QUIT" to quit.\n> ').upper()
    if response == 'QUIT':
        exit()
    ...
    for i in range(len(factors)):
        factors[i] = str(factors[i])

    print(f"{', '.join(factors)} are factors of {number}.")
    continue
